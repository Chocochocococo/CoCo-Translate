...
9. **YouTube 字幕：**
翻譯 YouTube 的 CC 字幕。影片要開啟 CC 字幕。
影片上的字幕可選「雙語」、「只譯文」、「只原文」或「不顯示」；字幕側欄可以另外開關。
想讓影片畫面保持乾淨的話，選「不顯示」、打開字幕側欄就好（CC 還是要開著，YouTube 才會下載字幕）。
會讀取整份字幕檔，把一個字一個字冒出來的字幕合成完整的句子，先整批翻好，播放時一次顯示一整句，不會一直跳動。
影片右邊會出現「字幕側欄」，列出整部影片的原文和譯文：正在播的那句會標亮，點一句就跳到那裡，選取文字可以查單字。
讀不到字幕檔時（例如 YouTube 改了取得方式），會改成翻譯畫面上正在顯示的 CC。
...
1. **一般：**
    ...
    5. YouTube 字幕：開關、影片上的字幕（雙語／只譯文／只原文／不顯示）、字幕側欄、字幕大小（以 YouTube 字幕設定的大小為準再放大縮小）、字幕位置。

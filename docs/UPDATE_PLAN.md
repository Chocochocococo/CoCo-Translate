...
原本是「一個樣式一個片段」送出：`He said <b>hello</b> to <i>her</i>.` 會拆成 5 段各自翻譯，前後文不連貫。
現在改成**一整段連同行內樣式一起送**，失敗時自動退回舊方式，沒有開關。

```
送出：He said <b id="g0">hello</b> to <i id="g1">her</i>.
收回：他昨天跟<i id="g1">她</i>打了個<b id="g0">招呼</b>。   ← 真實 Google 的結果，標籤照中文語序換位置
```

- **一段**＝連續的文字與行內元素（`<b>`、`<a>`、`<span>`、`<code>`……），遇到區塊元素或 `<br>` 就切開（小說網站用 `<br>` 分行時，每一行各自是一段）
- 行內元素換成帶 id 的標籤；圖片、表單元件等原封不動的東西變成 `<img id="xN">`
- `<code>`、`<kbd>`、`<samp>` 用 `<span>` 送出（Google 會把 `<code>` 裡的字當程式碼不翻）
- Google 用 `translateHtml`、Bing `textType=html`、DeepL `tag_handling=html`、Cloud Translation `format=html`、AI 翻譯在 prompt 裡交代保留標籤
- **套回譯文時只重複使用原本的節點**：文字節點改內容、元素只在同一層調整順序，不刪除也不跨層移動。Chrome 內建翻譯會把 React 之類的網站弄壞，就是因為直接換掉節點
- 每個 id 都要剛好出現一次、父子關係不變才套用；對不回去（翻譯服務弄丟或多加標籤）就退回逐片段翻
- 還原時把文字、節點順序完全恢復，新增的文字節點移除
- `<pre>` 與 textarea 的換行：HTML 模式會把換行吃掉（舊版也有這個問題），現在送出前換成 `<br>`，回來再換回來
- 自訂正規表達式與引號轉換只處理文字，不會動到標籤
- 超過 3000 字、沒有分段的一整塊直接用逐片段翻

測試：用 curl 對真實 Google 驗證 5 種情況（語序調換、巢狀、圖片、`&` 與 `<` 跳脫、換行）全部能正確解析；單元測試 30 個、Chromium 端對端 29 項全部通過。

...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  openApiModalBtn.addEventListener('click', () => {
    ...
    // 讀取先前設定
    browser.storage.local.get(['triggerTranslationSource', 'pageTranslationSource'], data => {
    ...
    document.getElementById('saveApiSelection').addEventListener('click', () => {
      ...
      browser.storage.local.set({
        ...
        browser.runtime.sendMessage({ type: 'UPDATE_TRIGGER_TRANSLATION_SOURCE', translationSource: triggerSource });
        browser.runtime.sendMessage({ type: 'UPDATE_PAGE_TRANSLATION_SOURCE', translationSource: pageSource });
  ...
  // 當 checkbox 狀態改變時，立刻存到 browser.storage.local
  useDiskCacheCheckbox.addEventListener('change', (e) => {
    const useDiskCache = e.target.checked;
    browser.storage.local.set({ useDiskCache: useDiskCache }, () => {
      if (browser.runtime.lastError) {
        console.error('儲存設定失敗，真他媽的：', browser.runtime.lastError);
      } else {
  ...
  enableCustomRegexCheckbox.addEventListener('change', (e) => {
    ...
    browser.storage.local.set({ enableCustomRegex: enabled }, () => {
      console.log('自訂正規表達式功能已', enabled ? '啟用' : '關閉');
      browser.runtime.sendMessage({ type: 'TOGGLE_CUSTOM_REGEX', enabled: enabled });
    });
  });

  enableSelectionButtonCheckbox.addEventListener('change', (e) => {
    const enableSelectionButton = e.target.checked;
    browser.storage.local.set({ enableSelectionButton: enableSelectionButton });
    browser.runtime.sendMessage({ type: 'UPDATE_SELECTION_BUTTON', enableSelectionButton: enableSelectionButton });
  });

  showOriginalTooltipCheckbox.addEventListener('change', (e) => {
    const showOriginalTooltip = e.target.checked;
    browser.storage.local.set({ showOriginalTooltip: showOriginalTooltip });
    browser.runtime.sendMessage({ type: 'UPDATE_SHOW_ORIGINAL_TOOLTIP', showOriginalTooltip: showOriginalTooltip });
  });
  ...
  saveBtn.addEventListener('click', () => {
    const selectedLanguage = targetLanguageSelect.value,
      ...
      if (selectedTriggerKey) browser.storage.local.set({ triggerKey: selectedTriggerKey });
      browser.storage.local.set({ 
      ...
      browser.runtime.sendMessage({ type: 'SET_TARGET_LANGUAGE', language: selectedLanguage });
      browser.runtime.sendMessage({ type: 'TOGGLE_TRANSLATION', isEnabled });
      browser.runtime.sendMessage({ type: 'UPDATE_SELECTION_BUTTON', enableSelectionButton: enableSelectionButton });
  ...
  translatePageBtn.addEventListener('click', () => {
    browser.tabs.query({ active: true, currentWindow: true }, tabs => {
      ...
      // 向內容腳本發送消息
      browser.tabs.sendMessage(tabId, { type: 'TRANSLATE_PAGE' });
      // 同時向背景頁發送消息，附帶 tabId
      browser.runtime.sendMessage({ type: 'TRANSLATE_PAGE', tabId: tabId });
    });
  });
  ...
  restorePageBtn.addEventListener('click', () => {
    browser.tabs.query({ active: true, currentWindow: true }, tabs => {
      const tabId = tabs[0].id;
      browser.tabs.sendMessage(tabId, { type: 'RESTORE_PAGE' });
      browser.runtime.sendMessage({ type: 'RESTORE_PAGE', tabId: tabId });
    });
  });

  const initRegexPatterns = () => {
    browser.storage.local.get(['regexPatterns'], data => {
  ...
  const togglePatternEnabled = e => {
    ...
    browser.storage.local.get(['regexPatterns'], data => {
      ...
      if (patterns[index]) {
        ...
        browser.storage.local.set({ regexPatterns: patterns }, notifyRegexUpdate);
      }
    });
  };
  ...
  const notifyRegexUpdate = () => {
    browser.tabs.query({}, tabs =>
      tabs.forEach(tab =>
        browser.tabs.sendMessage(tab.id, { type: 'UPDATE_REGEX_CACHE' })
      )
    );
  };
  ...
  document.getElementById('addPatternBtn').addEventListener('click', () => {
    browser.storage.local.get(['regexPatterns'], data => {
      ...
      browser.storage.local.set({ regexPatterns: patterns }, () => {
  ...
  document.getElementById('exportRegex').addEventListener('click', (e) => {
      browser.storage.local.get(['regexPatterns']).then(data => {
  ...
  enableFloatingButtonCheckbox.addEventListener('change', () => {
    ...
    browser.storage.local.set({ enableFloatingButton: isEnabled }, () => {
      console.log('Floating button setting saved:', isEnabled);
      browser.runtime.sendMessage({ type: 'TOGGLE_FLOATING_BUTTON', isEnabled });
    });
  });
  ...
  saveApiKeyBtn.addEventListener('click', () => {
    ...
    browser.storage.local.set({ googleApiKey: enteredKey }, () => {
  ...
  deleteApiKeyBtn.addEventListener('click', () => {
    browser.storage.local.remove('googleApiKey', () => {
  ...
  saveDeepLApiKeyBtn.addEventListener('click', () => {
    ...
    browser.storage.local.set({ deepLApiKey: enteredKey }, () => {
  ...
  deleteDeepLApiKeyBtn.addEventListener('click', () => {
    browser.storage.local.remove('deepLApiKey', () => {
  ...
  browser.storage.local.get(['deepLAccountType'], data => {
  ...
  deepLAccountTypeSelect.addEventListener('change', () => {
    ...
    browser.storage.local.set({ deepLAccountType: selectedType }, () => {
  ...
  saveMistralApiKeyBtn.addEventListener('click', () => {
    ...
    browser.storage.local.set({ mistralApiKey: enteredKey }, () => {
  ...
  deleteMistralApiKeyBtn.addEventListener('click', () => {
    browser.storage.local.remove('mistralApiKey', () => {
  ...
  if (cacheSizeDisplay) {
   ...
   if (clearCacheBtn) {
     clearCacheBtn.addEventListener('click', async () => {
       browser.tabs.create({ url: 'https://hypnotic-denim-05e.notion.site/1a1cbce1913780f7b850c0592ae7a0b9?pvs=4' });
      /* console.log('Popup: Clear Cache button clicked, fuck yeah!');
       try {
         ...
         browser.tabs.query({ active: true, currentWindow: true }, (tabs) => {
           if (tabs[0] && tabs[0].id) {
             browser.tabs.sendMessage(tabs[0].id, { type: 'CLEAR_TRANSLATION_CACHE' });
           }
         });
...
// 載入時設定下拉選單選擇值
browser.storage.local.get({ customPromptIndex: -1 }, data => {
...
// 讀取並更新自訂 prompt 下拉選單
browser.storage.local.get({ customPrompts: [] }, data => {
...
document.getElementById('saveCustomPromptBtn').addEventListener('click', () => {
  ...
  browser.storage.local.get({ customPrompts: [] }, data => {
    ...
    browser.storage.local.set({ customPrompts: prompts }, () => {
...
document.getElementById('promptSelect').addEventListener('change', e => {
  ...
  browser.storage.local.set({ customPromptIndex: index });
});

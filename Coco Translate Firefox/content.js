...
const applyRegexPatterns = text => new Promise(resolve => {
  ...
  browser.storage.local.get(['enableCustomRegex', 'regexPatterns'], data => {
...
const handleTranslation = async target => {
  ...
  loadingIndicator.src = browser.runtime.getURL('icons/loading.gif');
...
const createTranslationButton = () => {
  if (!selectionTranslationButton) {
    ...
    selectionTranslationButton.innerHTML = `<img src="${browser.runtime.getURL('icons/translation.png')}" style="width:24px;height:24px;" />`;
...
const getStoredPosition = () => {
  return new Promise((resolve) => {
    browser.storage.local.get('floatingButtonPos', (result) => {
      if (browser.runtime.lastError) {
        console.error("Fuck, getStoredPosition error:", browser.runtime.lastError);
        return resolve(null);
      }
...
const storePosition = (left, top) => {
  ...
  browser.storage.local.set({ floatingButtonPos: { leftPercent, topPercent } }, () => {
    if (browser.runtime.lastError) {
      console.error("Fuck, storePosition error:", browser.runtime.lastError);
    }
  });
};
...
const createFloatingButton = async () => {
  ...
  Object.assign(floatingButton.style, {
    ...
    borderRadius: '50%',
    border: 'none',
    background: `url('${browser.runtime.getURL('icons/inputtrans.png')}') no-repeat center`,
...
const createCopyButton = () => {
  ...
  Object.assign(copyButton.style, {
    ...
    width: '32px',
    height: '32px',
    background: `url('${browser.runtime.getURL('icons/copy.png')}') no-repeat center`,
    backgroundSize: 'contain',
    border: 'none',
    borderRadius: '50%',
    cursor: 'pointer',
...
const createTranslationBoxes = () => {
  ...
  Object.assign(translateBtn.style, {
    ...
    height: '32px',
    fontSize: '10px',
    background: `url('${browser.runtime.getURL('icons/transbtn.png')}') no-repeat center`,
    backgroundSize: 'contain',
    borderRadius: '50%',
    border: 'none',
    color: '#fff',
...
// 初始化參數
browser.storage.local.get(['inputTargetLanguage'], data => {
...
(function() {
  ...
  function showCustomMenu(e) {
    ...
    const buttons = [
      { 
        id: 'clearTriggled', 
        icon: browser.runtime.getURL('icons/letter-c.png'), 
      ...
      { 
        id: 'restorePage', 
        icon: browser.runtime.getURL('icons/letter-r.png'), 
        title: 'Restore Page',
        action: () => { restorePage(); },
      ...
      { 
        id: 'selectTranslate', 
        icon: browser.runtime.getURL('icons/letter-s.png'), 
      ...
      {
        id: 'pageTranslate',
        icon: browser.runtime.getURL('icons/letter-p.png'),
    ...
    buttons.forEach(btn => {
      ...
      button.addEventListener('click', (ev) => {
        ev.stopPropagation();
        btn.action();
        browser.runtime.sendMessage({ type: btn.messageType, tabId: currentTabId });
        hideCustomMenu();
      });

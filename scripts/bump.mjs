// 升版本號：package.json（打包時會寫進兩個 manifest）＋ 官網（docs/index.html）上顯示的版本
// 用法：
...
// 官網上寫著版本號的地方都標了 data-version
if (fs.existsSync(SITE)) {
  const html = fs.readFileSync(SITE, 'utf8');
  if (!/data-version>/.test(html)) {
    console.log('docs/index.html: 找不到 data-version，沒有更動');
  } else {
    fs.writeFileSync(SITE, html.replace(/(data-version>)[^<]*(<)/g, `$1${version}$2`));
    console.log(`docs/index.html: ${version}`);
  }
}
